...
<div id="dash"><span class="listcount">즐겨찾기 한 게시글 수 ${listcount}</span></div>
...
<nav id="nav1">
	<ul id="num">
			...
			<c:if test="${(empty find_field)&&(empty find_name)}">
				...
				<c:if test="${page >1}">
					<a href="bookmark?page=${page-1}">[이전]</a>&nbsp;
				</c:if>
				...
				<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
					...
					<c:if test="${a != page}">
						<%--현재 쪽번호가 선택 안된경우 --%>
						<a href="bookmark?page=${a}">[${a}]</a>&nbsp;
					</c:if>
				</c:forEach>

				<c:if test="${page>=maxpage}">[다음]</c:if>
				<c:if test="${page<maxpage}">
					<a href="bookmark?page=${page+1}">[다음]</a>
				</c:if>
			</c:if>
			...
			<c:if test="${(!empty find_field) || (!empty find_name)}">
				...
				<a href="bookmark?page=${page-1}&find_field=${find_field}&find_name=${find_name}">
					[이전]</a>&nbsp;</c:if>

				<%--쪽번호 출력부분 --%>
				<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
					<c:if test="${a == page}"><${a}></c:if><c:if test="${a != page}">
					<a href="bookmark?page=${a}&find_field=${find_field}&find_name=${find_name}">
						[${a}]</a>&nbsp;</c:if>
				</c:forEach>
